"""
@author: Zach
@time:   2022/7/21 下午3:05
@E-mail: one.bud@foxmail.com
@IDE:    PyCharm
@File:   train_license_letters.py
@Intro:  车牌字母部分训练和预测
"""
import os
import sys
...
x = tf.placeholder(tf.float32, shape=[None, SIZE])  # (?, 1280)
y_ = tf.placeholder(tf.float32, shape=[None, NUM_CLASSES])  # (?, 26)
x_image = tf.reshape(x, [-1, WIDTH, HEIGHT, 1])  # (?, 32, 40, 1)
...
if __name__ == '__main__' and sys.argv[1] == 'train':
    ...
    for i in range(0 + 10, NUM_CLASSES + 10):
        dir = './images/tf_car_license_dataset/train_images/training-set/letters/%s/' % i
    ...
    # 定义对应维数和各维长度的数组
    input_images = np.array([[0] * SIZE for i in range(input_count)])  # (3467, 1280) 共3467张图片
    input_labels = np.array([[0] * NUM_CLASSES for i in range(input_count)])  # (3467, 26)
    ...
    val_images = input_images.copy()
    val_labels = input_labels.copy()
    ...
    with tf.Session() as sess:
        ...
        b_fc1 = tf.Variable(tf.constant(0.1, shape=[512]), name='b_fc1')  # <tf.Variable 'b_fc1:0' shape=(512,) dtype=float32_ref>
