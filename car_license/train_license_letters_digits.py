"""
@author: Zach
@time:   2022/7/22 下午11:50
@E-mail: one.bud@foxmail.com
@IDE:    PyCharm
@File:   train_license_letters_digits.py
@Intro:  车牌最后5位字母数字部分训练和预测
"""
import os
import sys
import time
...
SAVER_DIR = 'train_saver/letters_digits/'
...
y_ = tf.placeholder(tf.float32, shape=[None, NUM_CLASSES])  # (?, 34)
...
def conv_layer(inputs, W, b, conv_strides, kernel_size, pool_strides, padding):
    ...
    # ksize: 由4个元素组成的一维int张量。输入张量的每个维度的窗口大小。
...
if __name__ == '__main__' and sys.argv[1] == 'train':
    # 第一次遍历图片目录是为了获取图片总数
    input_count = 0
    for i in range(0, NUM_CLASSES):  # 遍历字母和数字
        dir = './images/tf_car_license_dataset/train_images/training-set/letters_digits/%s/' % i
        for rt, dirs, files in os.walk(dir):
            for fn in files:
                input_count += 1

    # 定义对应维数和各维长度的数组
    input_images = np.array([[0] * SIZE for i in range(input_count)])  # (4285, 1280) 共4285张图片
    input_labels = np.array([[0] * NUM_CLASSES for i in range(input_count)])  # (4285, 34)
            ...
            for fn in files:
                ...
                for h in range(0, height):
                    for w in range(0, width):
                        # 通过这样的处理，例数字的线条变细，有利于提高识别准确率
                        if img.getpixel((w, h)) > 230:
                            input_images[index][w + h * width] = 0
                        else:
                            input_images[index][w + h * width] = 1
                input_labels[index][i] = 1  # index [0,4285) 每张图片对应类别的位置置为1
                index += 1
